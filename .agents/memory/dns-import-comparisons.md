...
**How to apply:** Serialize scan/import/configuration state updates, canonicalize equivalent DNS RDATA before comparing, flag missing/changed/import-only records separately, and require DNS-only service records when proxy status is known or explicitly rechecked when it is absent. Recovery wizards must derive progress from persisted state; replaying completed steps is strictly read-only, while only the active unfinished resume step may send changes.